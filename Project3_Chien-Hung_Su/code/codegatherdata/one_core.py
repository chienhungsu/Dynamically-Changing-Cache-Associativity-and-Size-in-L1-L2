...
single_thread_workloads = ['applu','bwaves_06','bzip2_source']
# 'applu','apsi', 'bwaves_06', 'bzip2_source', 'cactusADM_06', 'crafty'
#GLOBALS
bench_dir = "/home/chien/Desktop/SMTSIM/benchmarks/"
scr_dir =  "/home/chien/Desktop/611Project/shell/" 
exe = "smtsim"
exe_path = os.path.join("/home/chien/Desktop/SMTSIM/SMTSIM/build.linux-amd64/", exe)
sims_group = "/home/chien/Desktop/611Project/results/" 
thread_length = 1e7
ff_dist = 1e7
host = ''
...
def create_configurations():
	...
	c0= [32, 32, 64, 64, 64, 24, 1,32,1,32,2,1,1, 8, 4,4, 128]
	c1= [32, 32, 64, 64, 64, 24, 1,64,1,64,2,1,1, 8, 4,4, 128]
	c2= [32, 32, 64, 64, 64, 24, 2,32,2,32,2,1,1, 8, 4,4, 128]
	c3= [32, 32, 64, 64, 64, 24, 2,64,2,64,2,1,1, 8, 4,4, 128]
	c4= [32, 32, 64, 64, 64, 24, 1,32,1,32,2,1,1, 8, 4,4, 256]
	c5= [32, 32, 64, 64, 64, 24, 1,64,1,64,2,1,1, 8, 4,4, 256]
	c6= [32, 32, 64, 64, 64, 24, 2,32,2,32,2,1,1, 8, 4,4, 256]
	c7= [32, 32, 64, 64, 64, 24, 2,64,2,64,2,1,1, 8, 4,4, 256]
	c8= [32, 32, 64, 64, 64, 24, 1,32,1,32,2,1,1, 8, 4,8, 128]
	c9= [32, 32, 64, 64, 64, 24, 1,64,1,64,2,1,1, 8, 4,8, 128]
	c10= [32, 32, 64, 64, 64, 24, 2,32,2,32,2,1,1, 8, 4,8, 128]
	c11= [32, 32, 64, 64, 64, 24, 2,64,2,64,2,1,1, 8, 4,8, 128]
	c12= [32, 32, 64, 64, 64, 24, 1,32,1,32,2,1,1, 8, 4,8, 256]
	c13= [32, 32, 64, 64, 64, 24, 1,64,1,64,2,1,1, 8, 4,8, 256]
	c14= [32, 32, 64, 64, 64, 24, 2,32,2,32,2,1,1, 8, 4,8, 256]
	c15= [32, 32, 64, 64, 64, 24, 2,64,2,64,2,1,1, 8, 4,8, 256]
	...
	for c in configs_to_test:    
		...
		conf_dict['l2a']=c[15] # L2 assoc
		conf_dict['l2size']=c[16] # L2size
...
cd "%s"
%s -conffile "/home/chien/Desktop/SMTSIM/smtsim-scripts/workloads-list_ffs0.conf" \
...
""" % (bench_dir, exe_path,  sims_group+simout_name, input1, "%e" %ff_dist, input1, "%e" %thread_length,conf['l2size'],conf['l2a'],conf['ics'],conf['ica'],conf['dcs'],conf['dca'], conf['lsq'], conf['iqs'], conf['fqs'],conf['fb'],conf['fb'],conf['mci'],conf['mci'],conf['mii'],conf['mfi'],conf['mli'],conf['ipr'],conf['fpr'],conf['rob'],get_ceil_power_of_two(conf['rob']*8), sims_group, simout_name)
#add conf_dict['l2size']
    return script
...
def main():
	...
	for (input1) in single_thread_workloads:
		...
		for c in c_list:
			if t==16:
				t=0
			simout_name = get_simout_name(exe,input1,ff_dist,c)+ str(t)
			if t <=15:
				t =t +1
