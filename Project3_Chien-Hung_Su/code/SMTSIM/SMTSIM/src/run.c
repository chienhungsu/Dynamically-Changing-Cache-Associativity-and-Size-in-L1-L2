 ...
 */
    ...
    while(1)
    {
      /*  going through the pipeline back to front ensures that instructions
        ...
        cyc++;
	if((Contexts[0]->stats.instrs)>total1 && (Contexts[0]->stats.instrs)<test){
	printf("cyc: %s, instr: %s \n",fmt_i64(cyc),fmt_i64(Contexts[0]->stats.instrs));
	total1=total1+10000;
	test=test+10000;
	}
#ifdef DEBUG
	...
	//total1 += Contexts[0]->stats.instrs;
   // if(cyc=10){
	//printf("cba %s\n",cyc);//Contexts[i]->stats.instrs);
     // }
	//}   
/*
