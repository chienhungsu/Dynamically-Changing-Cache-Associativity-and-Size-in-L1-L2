...
process_l1fill_dfill(CacheRequest *creq)
{
    ...
    if (creq->drequestor) {
	   ...
	   //if(!nextinst){t=t+1;}
            ...
            meminst->mergeinst = NULL;
	    //printf("number %d,time %s",t,fmt_now());
	    //printf("Cnt:%d\ncache: time %s addr %s, L3 access %s: %s \n",cnt,fmt_now(), fmt_laddr(creq->base_addr),CacheAccessType_names[access_type],CacheLOutcome_names[cache_stat]);
            ready_time =
