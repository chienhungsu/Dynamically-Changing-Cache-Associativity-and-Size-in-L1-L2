...
import string
import socket
#from workload_combs_alphabet import two_thread_workloads
two_thread_workloads = [['applu', 'apsi'], ['art_470', 'bwaves_06'], ['bzip2_source', 'cactusADM_06']]

#GLOBALS
bench_dir = "/usr/local/shared/ECE641/benchmarks/alltogether/"
scr_dir = "/home/user_name/ECE611/two_core/" #--Please provide the exact path
exe = "smtsim"
exe_path = os.path.join("/usr/local/bin", exe)
sims_group = "/home/user_name/ECE611/results/"  #-Please provide the exact path
thread_length = 1e6
ff_dist = 1e6
host = ''
...
def gen_script(input1,input2, ff_dist, conf, thread_length, simout_name):
    script = '#!/bin/bash\n'
    global host
    if host == '':
...
cd "%s"
%s -conffile "/usr/local/shared/ECE641/benchmarks/conf/workloads-list_ffs0.conf" \
                              ...
                              -confexpr "WorkQueue/Jobs/job_2 = { start_time = 0.; workload = \"%s\"};"\
                              -confexpr "WorkQueue/max_running_jobs = 2;"\
                              -confexpr "Global/thread_length = %s;" \
                              -confexpr "Global/num_cores = 2;" \
                              -confexpr "Global/num_contexts = 2;" \
                              -confexpr "Global/ThreadCoreMap/t0 = 0;" \
                              -confexpr "Global/ThreadCoreMap/t1 = 1;" \
...
""" % (bench_dir, exe_path,  sims_group+simout_name, input1, "%e" %ff_dist, input2, "%e" %ff_dist, input1, input2,  "%e" %thread_length,conf['ics'],conf['ica'],conf['dcs'],conf['dca'], conf['lsq'], conf['iqs'], conf['fqs'],conf['fb'],conf['fb'],conf['mci'],conf['mci'],conf['mii'],conf['mfi'],conf['mli'],conf['ipr'],conf['fpr'],conf['rob'],get_ceil_power_of_two(conf['rob']*8), sims_group, simout_name)
    return script
    
def get_simout_name(exe,input1,input2,ff_dist,c):
    s = ""
...
def main():
    ...
    for (input1, input2) in two_thread_workloads:
        #if input != 'gamess_06_triazolium': 
        #    continue

        for c in c_list:
            simout_name = get_simout_name(exe,input1,input2,ff_dist,c)
            s = gen_script(input1,input2, ff_dist, c, thread_length, simout_name)
